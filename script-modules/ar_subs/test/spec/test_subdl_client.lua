...
-- can't leak in.
---------------------------------------------------------------------------
package.loaded["ar_subs.providers.subdl"] = nil
local provider = require "ar_subs.providers.subdl"
...
do
 ...
 do
   ...
   local src_path = mp_stub._test_root .. "/../scripts/ar_subs.lua"
   local f = io.open(src_path, "r")
   H.ok("scripts/ar_subs.lua readable for source inspection", f ~= nil)
   if f then
     local src = f:read("*a")
     f:close()
