-- ar_subs.config: load runtime configuration from .env + environment + script-opts.
--
...
function M.load(mp, options)
  ...
  local env_config = {
    ...
    subsource_api_key = os.getenv("SUBSOURCE_API_KEY") or dotenv.SUBSOURCE_API_KEY or "",
  ...
  local config = {
    ...
    tvdb_api_key = "",
    use_tvdb_cour = false,
    -- Keys below were previously read via opt_or()/tonumber() fallbacks
