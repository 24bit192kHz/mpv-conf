...
do
  ...
  local head_part = string.rep("\0", 512)
  local tail_part = string.rep("\1", 512)
  local buf = head_part .. tail_part  -- 1024 bytes exactly
...
do
  ...
  math.randomseed(42)
  local pat_a = {}
  for i = 1, 512 do pat_a[i] = string.char(math.random(1, 255)) end
  pat_a = table.concat(pat_a)

  math.randomseed(99)
  local pat_b = {}
  for i = 1, 512 do pat_b[i] = string.char(math.random(1, 255)) end
  pat_b = table.concat(pat_b)

  local pad = string.rep("\0", 1536)
...
do
  ...
  local head_part = string.rep("\0", 512)
  local tail_part = string.rep("\1", 512)
  local middle = string.rep("\2", 256 * 1024 - 1024)
...
do
  ...
  local f = io.open(tmp_path, "wb")
  if f then
    f:write(string.rep("\0", 512))  -- 512 bytes, below 1024 minimum
    f:close()
    local hash = oshash.compute_file(tmp_path)
    H.eq("file < 1024 bytes returns nil", hash, nil)
    os.remove(tmp_path)
  end
end

---------------------------------------------------------------------------
-- (4b) File at exactly 1024 bytes (boundary) → returns hash
---------------------------------------------------------------------------
do
  H.reset()
  ...
  local f = io.open(tmp_path, "wb")
  if f then
    f:write(string.rep("\0", 1024))  -- exactly 1024 bytes
    f:close()
    local hash = oshash.compute_file(tmp_path)
    H.ok("file at exactly 1024 bytes returns hash", type(hash) == "string" and #hash == 16)
    os.remove(tmp_path)
  end
end
...
do
  ...
  H.eq("data < 1024 bytes returns nil", h, nil)
end
...
do
  H.reset()
  -- Exactly 1024 bytes: head = first 512, tail = last 512 (disjoint)
  local buf = string.rep("\x42", 512) .. string.rep("\x37", 512)
  local h = oshash.compute_string(buf)
  H.ok("minimum 1024-byte buffer produces a hash", type(h) == "string" and #h == 16)
end
