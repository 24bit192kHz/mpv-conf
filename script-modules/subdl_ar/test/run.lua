#!/usr/bin/env lua
-- Zero-dependency test runner for subdl_ar modules.
--
...
--
-- Usage:  lua script-modules/subdl_ar/test/run.lua
...
local _script_modules = _module_root:match("^(.*)/subdl_ar$") or _module_root
...
--   require "subdl_ar.util.x" -> <script_modules>/subdl_ar/util/x.lua
--   require "subdl_ar.config" -> <script_modules>/subdl_ar/config.lua
--   require "subdl_ar"        -> <script_modules>/subdl_ar/init.lua
...
-- Also expose config root so subdl_ar.config (if required during a spec) can
...
print(string.format("subdl_ar test runner: %d spec file(s)", #spec_files))
print(string.rep("-", 60))
